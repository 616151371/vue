import Vue from 'vue'
import Router from 'vue-router'
// in development env not use Lazy Loading,because Lazy Loading too many pages will cause webpack hot update too slow.so only in production use Lazy Loading
/* layout */
import Layout from '../views/layout/Layout'

const _import = require('./_import_' + process.env.NODE_ENV)
Vue.use(Router)
export const constantRouterMap = [
  {path: '/login', component: _import('login/index'), hidden: true},
  {path: '/404', component: _import('404'), hidden: true},
  {
    path: '/',
    component: Layout,
    redirect: '/dashboard',
    name: '首页',
    hidden: true,
    children: [{
      path: 'dashboard', component: _import('dashboard/index')
    }]
  }
]
export default new Router({
  // mode: 'history', //后端支持可开
  scrollBehavior: () => ({y: 0}),
  routes: constantRouterMap
})
export const asyncRouterMap = [
  {
    path: '/system',
    component: Layout,
    redirect: '/system/',
    name: '',
    meta: {title: '业务管理', icon: 'table'},
    children: [
      {
        path: 'article',
        name: '教研室管理',
        component: _import('article/article'),
        meta: {title: '教研室管理', icon: 'example'},
        menu: 'article'
      }, {
        path: 'leave',
        name: '考勤管理',
        component: _import('leave/leave'),
        meta: {title: '考勤管理', icon: 'example'},
        menu: 'leave'
      }, {
        path: 'workload',
        name: '工作量管理',
        component: _import('workload/workload'),
        meta: {title: '工作量管理', icon: 'example'},
        menu: 'workload'
      }, {
        path: 'class',
        name: '教学材料管理',
        component: _import('class/class'),
        meta: {title: '教学材料管理', icon: 'example'},
        menu: 'class'
      }, {
        path: 'student',
        name: '学生管理',
        component: _import('student/student'),
        meta: {title: '学生管理', icon: 'example'},
        menu: 'student'
      }
    ]
  },
  {
    path: '/user',
    component: Layout,
    redirect: '/user/',
    name: '',
    meta: {title: '系统管理', icon: 'table'},
    children: [
      {
        path: '',
        name: '用户列表',
        component: _import('user/user'),
        meta: {title: '用户列表', icon: 'user'},
        menu: 'user'
      },{
        path: 'role',
        name: '权限管理',
        component: _import('user/role'),
        meta: {title: '权限管理', icon: 'password'},
        menu: 'role'
      }
    ]
  },
  {path: '*', redirect: '/404', hidden: true}
]
